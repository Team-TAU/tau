<template>
  ...
  <div class="grid">
    <div class="col-6">
      <Panel class="dark-header" header="Streamer Status">
        <template #icons>
          <button class="p-panel-header-icon p-link p-mr-2" @click="addStreamer()">
            <span class="pi pi-plus"></span>
          </button>
        </template>
        <DataTable :value="data" stripedRows>
          <Column field="streaming" headerStyle="width: 6rem; text-align: center" header="Status"
            bodyClass="text-center" headerClass="text-center">
            <template #body="{ data }">
              <Button icon="pi pi-video" class="p-button-rounded p-button-success" v-if="data.streaming" />
              <Button icon="pi pi-times" class="p-button-rounded p-button-outlined not-streaming"
                v-if="!data.streaming" />
            </template>
          </Column>
          ...
          <Column field="delete" headerStyle="width: 4rem; text-align: center" header="">
            <template #body="{ data }">
  ...
  <Dialog v-model:visible="displayAddModal" header="Add Streamer" :modal="true" :closable="false" style="width: 450px">
    <twitch-user v-model:valueId="newStreamer.user_id" v-model:valueName="newStreamer.user_name"
      v-model:valueLogin="newStreamer.user_login" label="Streamer"></twitch-user>
    <template #footer>
      <Button label="Cancel" icon="pi pi-times" @click="closeAddForm" class="p-button-text" />
      <Button label="Submit" icon="pi pi-check" @click="submitAddForm" class="p-button-text" />
    </template>
  </Dialog>
</template>
