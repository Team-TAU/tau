...
def test_irc_client_parse_message_on_channel_points_redemption():
    ...
    # Validate a message with no emotes has an empty list of emotes
    ...
    # Validate emote-only exists on an emote only message
    ...
    assert 'emotes' in msg['tags']
    assert msg['tags']['emotes'] == [{'id': '25', 'positions': [[0, 4]]}]
    assert msg['tags']['msg-id'] == 'highlighted-message'
    assert msg['tags']['emote-only'] == '1'

    # Validate emote-only doesn't exist on a message with emotes and non-emotes
    ...
    # Validate that the custom reward id/params are parsed properly
