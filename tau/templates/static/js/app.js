...
window.onload = (event) => {
    setupJsonWebsocket('ws://localhost:8000/ws/tau-status/', handleStatusMessage)
    setupJsonWebsocket('ws://localhost:8000/ws/twitch-events/', handleEventMessage);
    const prModal = document.getElementById('testPointsRedemptionModal');
    prModal.addEventListener('shown.bs.modal', function () {
        ajaxGet('http://localhost:8000/api/v1/channel-point-rewards/').subscribe(resp => {
    ...
    tokenModal.addEventListener('shown.bs.modal', function () {
        ajaxGet('http://localhost:8000/api/v1/tau-user-token/').subscribe(resp => {
...
const replayEvent = (id) => {
    const sub = ajaxPost(`http://localhost:8000/api/v1/twitch-events/${id}/replay/`, {}).subscribe(resp => {
        console.log(resp);
    })
}